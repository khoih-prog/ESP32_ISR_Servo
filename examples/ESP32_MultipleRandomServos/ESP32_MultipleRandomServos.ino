/****************************************************************************************************************************
 * examples/ESP32_MultipleRandomServos.ino
 * For ESP32 boards
...
void loop() 
{
  ...
  for (int index = 0; index < NUM_SERVOS; index++)
  {
    ...
    Serial.print("Servos idx = " + String(index) + ", act. pos. (deg) = " + String(ESP32_ISR_Servos.getPosition(ISR_servo[index].servoIndex)) );
    Serial.println(", pulseWidth (us) = " + String(ESP32_ISR_Servos.getPulseWidth(ISR_servo[index].servoIndex)) );
 }
